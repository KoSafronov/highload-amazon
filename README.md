...
### **9.1. Глобальная балансировка нагрузки и отказоустойчивость**

#### **9.1.1. Geo-Distribution (Геораспределение)**

- **Датацентры**:
  - **Восточное побережье (Вирджиния, Нью-Йорк)** – для пользователей США и Европы.
  - **Западное побережье (Калифорния, Орегон)** – для пользователей западного побережья и Азии.
  - **Центральный регион (Техас, Иллинойс)** – для балансировки между востоком и западом.
- **Межрегиональная репликация**:
  - Базы данных (PostgreSQL, MongoDB, ElasticSearch) реплицируются между датацентрами.
  - Кэши (Redis, EVCache) синхронизируются через **Kafka** или **CRDT (Conflict-free Replicated Data Types)**.

#### **9.1.2. DNS-балансировка и Anycast**

- **Amazon Route 53** (latency-based routing) – автоматически направляет пользователей к ближайшему датацентру.
- **Cloudflare** – защита от DDoS, кэширование статики, резервный DNS.
- **Anycast IP** – для API и критичных сервисов (например, платежей).

#### **9.1.3. Автоматическое переключение (Failover)**

- **Health Checks** (Prometheus + Grafana) – мониторинг состояния сервисов.
- **Автоматический rerouting** через **Envoy Proxy** или **AWS ALB/NLB**.
- **Circuit Breaker** (Hystrix/Resilience4j) – изоляция упавших сервисов.

---

### **9.2. Резервирование критичных компонентов**

| **Компонент**      | **Стратегия резервирования**                | **Описание**                                     |
| ------------------ | ------------------------------------------- | ------------------------------------------------ |
| **PostgreSQL**     | Master-Slave + Patroni, шардирование        | Автоматический failover, реплики для чтения.     |
| **MongoDB**        | Replica Set, шардирование                   | Автоматическое переключение на вторичный узел.   |
| **Redis**          | Cluster Mode, репликация + Sentinel         | Горячий standby, автоматическое восстановление.  |
| **ElasticSearch**  | Реплики шардов, cross-cluster replication   | Поиск продолжает работать даже при потере узлов. |
| **Kafka**          | Репликация партиций (min.insync.replicas=2) | Сообщения не теряются при сбое брокера.          |
| **S3 (хранилище)** | Multi-AZ + версионирование                  | Автоматическое восстановление данных.            |

---

### **9.3. Graceful Degradation (Постепенная деградация)**

#### **9.3.1. Недоступность сервиса модерации**

- **Карточки товаров** временно видны только продавцу (`is_hidden=true`).
- **Очередь модерации** накапливается в Kafka (`moderation_jobs`).

#### **9.3.2. Недоступность рекомендаций**

- Используются **последние кэшированные рекомендации** из Redis.
- Если сервис упал – поиск работает через **ElasticSearch** (без персонализации).

#### **9.3.3. Недоступность поиска**

- **Базовый поиск** через PostgreSQL/MongoDB (без фасетов и ранжирования).
- **Геопоиск** заменяется на фильтр по городу (`address LIKE '%New York%'`).

#### **9.3.4. Недоступность платежей**

- **Корзина сохраняется в Redis**, но заказ не оформляется.
- Пользователю показывается сообщение: _"Платежи временно недоступны, попробуйте позже"_.

---

### **9.4. Асинхронные паттерны**

#### **9.4.1. Saga Pattern**

- Для **транзакций между микросервисами** (например, создание заказа → резерв товара → оплата).
- **Оркестратор (Kafka + Camunda)** управляет компенсирующими транзакциями.

#### **9.4.2. Event-Driven Architecture**

- **Kafka** – центральный шинный сервис для событий:
  - `order_created` → логистика, аналитика.
  - `review_added` → пересчет рейтингов.
  - `payment_processed` → обновление баланса.

#### **9.4.3. CQRS (Command Query Responsibility Segregation)**

- **Запись** (`Command`) – в PostgreSQL.
- **Чтение** (`Query`) – из ElasticSearch/Redis (для ускорения).

---

### **9.5. Мониторинг и автоматическое восстановление**

#### **9.5.1. Prometheus + Grafana**

- Сбор метрик: **RPS, latency, ошибки, загрузка CPU/RAM**.
- **Alerting** (PagerDuty / Slack) при аномалиях.

#### **9.5.2. Chaos Engineering (Gremlin)**

- **Тестирование отказоустойчивости**:
  - Рандомные остановки сервисов (`kill -9`).
  - Эмуляция сетевых задержек (`tc netem`).

#### **9.5.3. Автомасштабирование (Kubernetes HPA / AWS Auto Scaling)**

- **Горизонтальное масштабирование** сервисов при росте нагрузки.
- **Вертикальное масштабирование** БД (Aurora Auto Scaling).
...
[Изучить схему проекта можно по ссылке](10_1.md)
